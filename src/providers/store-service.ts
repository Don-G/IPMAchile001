    ...
    purchaseProduct(product, resultCallback, errorCallback){
      ...
      if (typeof iap != 'undefined') {
          ...
          if (product.type === 'consumable') {
            ...
            if(product.productId == "quiz2_f_version"){alert('full version purchased'); this.dbservice.setProperty('fullversion','true'); this.dbservice.setFullVersion(); this.dbservice.setProperty('admob','false'); }
            if(product.productId == "quiz2_no_ads"){alert('no ads purchased'); this.dbservice.setProperty('admob','false');}
            if(product.productId == "quiz2_exam_x1"){alert('Exam ID: 4 Unlocked');this.dbservice.unlockExamById(4);} //Exam Id shall be passed as parameter
            if(product.productId == "quiz2_exam_x3"){
                //This is the action that we call when purchasing this product Exam x3.
  ...
  updateConsumable(product){
    ...
    switch(product.productId) {
      case 'quiz2_expla_x5':
            ...
            break;
      case 'quiz2_expla_x10': // this has to corresponds with the product id configured in productIds array
            addQuantity = 10;
            consumable = 'explanationq';
            break;
      case 'quiz2_expla_x50': // this has to corresponds with the product id configured in productIds array
            addQuantity = 50;
            consumable = 'explanationq';
            break;
    }
